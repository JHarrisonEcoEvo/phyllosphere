...
write.csv(var.imp, file = paste("variableImportanceshannonsISD_16s_withSite.csv"), row.names = T)
...
write.csv(var.impR, file = paste("variableImportanceReducedshannonsISD_16s_withSite.csv"), row.names = T)

write.csv(out, file = paste("results_shannonosISD_16s_withSite.csv"), row.names = F)
