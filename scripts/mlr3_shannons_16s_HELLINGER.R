...
var.imp <- var.imp[rev(order(var.imp$tained_at.model.learner.model.regr.ranger.model.variable.importance)),]


names(var.imp) <- c("Feature","Decline in model performance")
xtable::xtable(
  label = "stable: featureImp_shannons16s",
  
  caption = "Output from random forest model of bacterial Shannon's diversity. Features ranked by importance (largest number corresponds with most important feature). To determine importance, features were permuted, the model rerun, and the decline in model performance (mean squared error) assayed (as implemented via the Ranger R package).",
  var.imp[1:15,])


#write.csv(var.imp , file = paste("variableImportanceshannonsISD_16s_raw.csv"), row.names = T)
write.csv(var.imp , file = paste("variableImportanceshannonsISD_16s_rawcsv"), row.names = T)
#write.csv(out , file = paste("results_shannonsISD_16s_rawNOHOSTDATA.csv"), row.names = T)

write.csv(out, file = paste("results_shannonsISoutD_16s_raw.csv"), row.names = F)
