...
#For convenience make response variable. THIS IS THE SUM COUNTS
#Note that many samples don't have much besides non-target reads
summary(rowSums(merged_dat[,grep("Zotu", names(merged_dat))]-1))
response_taxon <- rowSums(merged_dat[,grep("Zotu", names(merged_dat))]-1) / merged_dat$ISD

#Get rid of stuff we don't need in merged_dat 
merged_dat <- data.frame(response_taxon, merged_dat[,names(merged_dat) %in%
                                                      c("sample",
                                                        "area_cm2"                                                 
                                                        , "mass_extracted_g"                                         
                                                        , "leaves_extracted"                                         
...
write.csv(var.imp, file = paste("variableImportance_abundance_16S.csv", sep = ""), row.names = T)
...
write.csv(var.impR, file = paste("variableImportanceReduced_abundance_16S.csv", sep = ""), row.names = T)

write.csv(out, file = paste("results_abundance_16S.csv", sep = ""), row.names = F)
