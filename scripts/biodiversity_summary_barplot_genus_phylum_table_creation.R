...
#match order
merged_dat <- merge(metadat16s, dat16s, by.x = "sample", by.y = "sample", all.y = T)
dim(merged_dat)

head(names(merged_dat),206)
tail(names(merged_dat),206)
names(merged_dat)[length(merged_dat)-4]

merged_dat[,202:(length(merged_dat)-4)] <- merged_dat[,202:(length(merged_dat)-4)] / merged_dat$ISD
datr16s <-  vegan::decostand(merged_dat[,202:(length(merged_dat)-4)], method = "hellinger")
...
head(names(merged_dat), 203)

merged_dat[,202:(length(merged_dat)-2)] <- merged_dat[,202:(length(merged_dat)-2)] / merged_dat$ISD
merged_dat[,202:(length(merged_dat)-2)]  <-  vegan::decostand(merged_dat[,202:(length(merged_dat)-2)], method = "hellinger")
...
pdf(width = 5,height = 6, file = "./visuals/phyla_its_compartment_barplot_CountData.pdf")
# Get the stacked barplot
par(mar = c(3,6,1,1), oma = c(2,2,2,2))
barplot(as.matrix(wide[,2:3]), 
        col=colors, 
...
pdf(width = 12,height = 9, file = "./visuals/phyla_its_taxon_final_barplot_CountData.pdf")
# Get the stacked barplot
par(mar = c(3,6,1,1), oma = c(16,3,1,1))
a <- barplot(as.matrix(data_wide_t), 
             col = colors,
