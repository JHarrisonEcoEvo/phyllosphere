...
if(isdNorm_tf == "T"){
  ...
  taxa[,3:length(taxa)]  <- vegan::decostand(taxa[,3:(length(taxa)-4)],
                                             method = "hellinger")
}
...
write.csv(var.imp, file = paste("noduds_variableImportance", focal_taxon, isdNorm_tf, "16S.csv", sep = ""), row.names = T)
...
#write.csv(var.impR, file = paste("variableImportanceReduced", focal_taxon,isdNorm_tf,  "16S.csv", sep = ""), row.names = T)
write.csv(out, file = paste("noduds_results", focal_taxon, isdNorm_tf, "16S.csv", sep = ""), row.names = T)
