...
write.csv(out , file = paste("results_shannonsISD_its_raw_EP_ONLY.csv"), row.names = T)
...
engineered_df <- data.frame(forgraphing$data(rows = 1:dim(merged_dat)[1], cols = colnames(merged_dat)))
...
pdf(width = 7, height = 8, file = "./visuals/varImp.div.its_ep.pdf")
...
p4 <- tained_at$model$learner$model$regr.ranger$model %>%  
  partial(pred.var = "shannons_flora", 
          train = engineered_df[,!(names(engineered_df) %in% c("sample", "merged_dat$rich", "rich","richISDstratum"))]) %>%
  autoplot(smooth = TRUE,  
           train = engineered_df[,!(names(engineered_df) %in% c("sample", "merged_dat$rich", "rich","richISDstratum"))],
           rug = TRUE, 
...
pdf(width = 10, height = 10, file = "./visuals/pdp_rich_its_ep.pdf")
