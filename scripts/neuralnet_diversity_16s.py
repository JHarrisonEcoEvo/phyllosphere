...
model.add(Dense(20,input_dim=strat_train_set.shape[1]-1,activation='relu'))
...
#Adding a dropout layer really seems to help the model's performance stabilize
...
#Stop training a bit early to prevent overfitting
...
history =  model.fit(X_array, Y_array, epochs=200, batch_size=50, callbacks=[es_callback])

#Learn a bit about model structure
model.summary()
#Saving this model, because it is performing well and don't want to lose weights
#model.save('../models/nn_20_10_dropout_5_1.keras')
...
r2 = r2_score(Y_arraytest, y_pred_test)
r2
...
plt.savefig("../visuals/NN_shannons16s.pdf")
