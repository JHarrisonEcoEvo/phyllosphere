...
#COMPARTMENT ########################
merged_dat16s <- merged_dat16s[merged_dat16s$compartment == "EN",]
...
#COMPARTMENT ########################
merged_datits <- merged_datits[merged_datits$compartment == "EN",]
...
###########################
###########################
rm(list=setdiff(ls(), c("dat_e16s", "datrits")))
library(ecodist)
library(vegan)
library(dendextend)
...
#COMPARTMENT ########################
merged_dat16s <- merged_dat16s[merged_dat16s$compartment == "EP",]
...
datr16s_ep <-  vegan::decostand(comboHost[,2:length(comboHost)], 
                             method = "hellinger")

dat_e16s_ep <- dist(x  = as.matrix(datr16s_ep), method = "euclidean", upper = T)
...
datrits_ep <-  vegan::decostand(comboHostits[,2:length(comboHostits)], 
                             method = "hellinger")

datrits_ep <- dist(x  = as.matrix(datrits_ep), method = "euclidean", upper = T)

mantel(dat_e16s_ep, dat_e16s)
mantel(datrits_ep, datrits)

mantel(datrits_ep, dat_e16s_ep)
mantel(dat_e16s, datrits)
