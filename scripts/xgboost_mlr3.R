...
# phyllo_task$col_roles$feature <- names(merged_dat)[which(names(merged_dat)=="area_cm2") : 
#                                                      which(names(merged_dat)=="phenology_vegetative")]
...
# impute? #
##########
#XGBOOST doesn't require imputation
#
# install.packages("missRanger")
# library(missRanger)
# 
# #First see proportion of missigness for each variable.
# 
# missing <- NA
# index <- NA
# for(i in 3:54){
#   if("TRUE" %in% names(table(is.na(merged_dat[,i]))) & "FALSE" %in% names(table(is.na(merged_dat[,i])))){
#     missing[i]<- table(is.na(merged_dat[,i]))[2] /(table(is.na(merged_dat[,i]))[1] + table(is.na(merged_dat[,i]))[2])
#     index[i] <- i
#     print(paste(i, " has this percentage of missing data: ",
#     table(is.na(merged_dat[,i]))[2] /(table(is.na(merged_dat[,i]))[1] + table(is.na(merged_dat[,i]))[2]),
#     "%", sep = ""))
#   }
# }
# o <- data.frame(missing, index)
# names(merged_dat)[o$index[o$missing > 0.15]]

#Deciding against imputing these few variables, since I am curious about them and
#XGBoost should be able to deal.
