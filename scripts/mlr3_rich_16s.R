...
X<- read.csv("./processedData/16smetadat_wrangled_for_post_modeling_analysis_withRich.csv")
...
write.csv(var.imp , file = paste("variableImportancerich_16s_rawcsv"), row.names = T)
write.csv(out, file = paste("results_rich_16s_raw.csv"), row.names = F)
...
merged_dat <- data.frame(merged_dat$sample, merged_dat[,-which(names(merged_dat)=="sample")])
#Convert to numeric (makes it easier when doing imputing)
for(i in 1:length(merged_dat)){
  merged_dat[,i] <- as.numeric(merged_dat[,i])  
}
...
dat2 = rfImpute(x=merged_dat[,c(2:40,42)], y = merged_dat$rich) 
...
rf = randomForest(
  ...
  y = dat2$`merged_dat$rich`[train],						#response
...
plot(out, dat2$`merged_dat$rich`[test])
cor.test(out, dat2$`merged_dat$rich`[test])
library(Metrics)
rmse(out, dat2$`merged_dat$rich`[test])
