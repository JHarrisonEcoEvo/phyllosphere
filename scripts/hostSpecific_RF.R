...
#Get taxa that were sampled more than 30 times. 
focalhosts <- names(table(X$taxon_final))[table(X$taxon_final) > 30]
#remove uknowns
focalhosts <- focalhosts[-grep("Unknown", focalhosts)]

#SUBSET TO JUST THESE TAXA TO SAVE TIME.
X <- X[X$taxon_final %in% focalhosts,]
