...
# )
params <- ps(
  classif.ranger.num.trees = p_int(lower = 50, upper = 400, tags = "budget"), #number of trees in the dark forest
  classif.ranger.sample.fraction = p_dbl(lower = 0.01, upper = 0.3), #The sample.fraction parameter specifies the fraction of observations to be used in each tree. Smaller fractions lead to greater diversity, and thus less correlated trees which often is desirable.
  #classif.ranger.splitrule = p_fct(levels = c("extratrees", "gini")),
  classif.ranger.min.node.size = p_int(lower = 1, upper = 25)
)
...
at <- AutoTuner$new(
  ...
  measure = msr("classif.mcc"),#msr("regr.rsq"), #mcc is Matthew's cor. coef.
  terminator = trm("none"),
...
table(rr$prediction()$response) #just predicted everything was zero
...
write.csv(out, file = paste("HOSTresults", focal_taxon, "_", focal_host, "ITS_OCCUPANCY.csv",  sep = ""), 
          row.names = F)
...
#Only do this if mcc is > 0
if(mcc > 0){
  tained_at <- at$train(phyllo_task)
  
  var.imp <- data.frame(tained_at$model$learner$model$classif.ranger$model$variable.importance)
  write.csv(var.imp, file = paste("HOSTvariableImportance", focal_taxon, "_", focal_host, "ITS_OCCUPANCY.csv", sep = ""), 
            row.names = T)
}
